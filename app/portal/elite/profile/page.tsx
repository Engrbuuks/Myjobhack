...
export default async function EliteProfile() {
  ...
  return (
    <>
      <PageHeader title="Your profile" sub="The same four axes drive your matching — with Elite priority on top." />
      <ProfileWizard profile={profile!} talent={talent!}
        expertiseIds={(expertise ?? []).map((e) => e.taxonomy_id)}
        taxonomies={taxonomies ?? []} resumeName={resumeName} />
    </>
  );
}
