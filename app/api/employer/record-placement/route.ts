...
export async function POST(request: Request) {
  ...
  let feeAmount = 0; let feeBasis = "flat";
  if (isElite) {
    feeBasis = "percent";
    ...
    feeAmount = Math.round(salary * (pricing.placement_elite_percent / 100));
  } else {
    feeBasis = "flat";
  ...
  const { data: inv } = await admin.from("invoices").insert({
    ...
    notes: isElite
      ? `Elite placement — ${pricing.placement_elite_percent}% of monthly salary.`
  ...
  const { data: placement, error } = await admin.from("placements").insert({
    ...
    fee_amount: feeAmount, fee_basis: feeBasis, currency: "NGN",
  ...
  return NextResponse.json({
    ...
    message: isElite
      ? `Elite placement recorded. Fee: ₦${feeAmount.toLocaleString()} (${pricing.placement_elite_percent}% of salary). Invoice ${inv?.number} raised.`
      : `Placement recorded. Flat fee ₦${feeAmount.toLocaleString()}. Invoice ${inv?.number} raised.`
  });
}
